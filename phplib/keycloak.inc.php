...
function get_keycloak_admintoken(){
    $conf = getConf(__DIR__."/../../conf/oauth2_admin.conf");
    $clientUser   = $conf[0];
    $clientSecret = $conf[1];
    ...
    #curl -X POST 'https://inb.bsc.es/auth/realms/master/protocol/openid-connect/token'  -H "Content-Type: application/x-www-form-urlencoded" -d "username=admin"  -d 'password=7fU!3g_54-'  -d 'grant_type=password'  -d 'client_id=admin-cli'
...
function get_keycloak_user($username,$token){
    ...
    #curl -v  -X GET -H "Authorization: Bearer  $token" -H "Accept: application/json" https://inb.bsc.es/auth/admin/realms/mug/users?username=laiacodo@gmail.com
...
function update_keycloak_user($userId,$userData,$token){
    ...
    #curl -v  -X PUT -H "Authorization: Bearer  $1" -H "Accept: application/json" -H "Content-Type: application/json" -d '{ "attributes": { "mug_id": ["MuGUSER59647a6c60244"] }}' https://inb.bsc.es/auth/admin/realms/mug/users/ad9ced86-38f5-4027-8338-d18a3b08990e    
