...
function formatData($data) {
		...
		if (isset($data['tool']) ){
			$tool = $GLOBALS['toolsCol']->findOne(array('_id' => $data['tool']));
			if (!empty($tool))
				//$data['viewResultsLink']="<li><a href=\"tools/".$data['tool']."/output.php?project=".$data['_id_URL']."\"><i class=\"fa fa-file-text\"></i> View Results</a></li>";
		    $data['viewResultsLink']="<li><a href=\"tools/loading_output.php?project=".$data['_id_URL']."&tool=".$data['tool']."\"><i class=\"fa fa-file-text\"></i> View Results</a></li>";
		}
		//analyses tool
		if (isset($data['tool'])){
			$tool = $GLOBALS['toolsCol']->findOne(array('_id' => $data['tool']));
...
function processPendingFiles($sessionId,$files){
	...
	foreach ($lastjobs as $job){
		...
		if (count($jobSGE)){
			...
			$fileDummy = Array(
				...
				'visible' => 1,
				'tool'    => $job['toolId'],
				'parentDir'=> getGSFileId_fromPath(fromAbsPath_toPath($job['working_dir'])),
				'description'=> $descrip,
				'pending' => $jobSGE['state'],
				'shPath'  => fromAbsPath_toPath($job['config_file']),
		...
		//processing job non running anymore
			...
			// build output list merging stageout_file and tool defintion data

			$outs_files = build_outputs_list($tool,$job['stageout_file']);
			...
			foreach ($outs_files as $out_name => $outs_data){
			    if ($debug)
			    	print "<br><br/><br/>REGISTERING output_files with NAME = $out_name <br/>";
			    ...
			    foreach ($outs_data as $out_data){
			        if ($debug)
					print "<br/>----->>> OUTPUT ITEM<br/>";

		    		//check requirement : required
				if (!isset($out_data['file_path'])){
					if ($is_required){
						$_SESSION['errorData']['Error'][]="Expected tool outfile '$out_name' has no 'file_path' attribute. Execution failed?";
						print "ERROR<br>";	
				...
				$fileId   = getGSFileId_fromPath($outPath);
			        ...
			        if ($debug)
					print "PID = [$pid] OUT=$outPath <br/>";			

 
				//validate new file data
				$out_mug = validateMugFile($out_data,true);
			        if ($debug)
					print "<br>Out file model validation returned ".count($out_mug)."<br>";


				// job successfully finished and already in mongo. Update medatada

				if ($fileId){
					$_SESSION['errorData']['Warning'][] = "Output file '".basename($rfn)."'for '$outs_data' is already registered. Doing nothing.";
					/*
				...
				// job successfully finished but not yet on mongo
					...
					if (is_array($fileInfo)){
						$filesPending[$fileInfo['_id']]=$fileInfo;
			        		if ($debug)
							print "<br/>SAVED successfully ".$fileInfo['_id']."!<br/>";
					}
					if($fileInfo == "0")
						$_SESSION['errorData']['error'][]="Cannot register the LOG file of the failed execution of ".basename($outPath);
					
				// job successfully finished but file metada not valid
					
				}elseif (is_file($rfn) && !$out_mug) {
			        	if ($debug){
						print "JOB $pid FINISHED BUT INVALID FILE ($outPath). SET ERROR <br>";
	                			print "<br/><br>The invalid file is:<br/>";
				                var_dump($out_mug);
					}
					$_SESSION['errorData']['Error'][]="Output file '".basename($rfn)."' for '$out_name' cannot be registered. Missing required metadata";
					$job_in_err=1;

				}else {
			        	if ($debug){
						print "JOB $pid FINISHED BUT NO EXPECTED OUTFILE '$rfn' FOUND  IN DISK. Set ERROR<br>";
					}
					$job_in_err=1;
				}
			    }
			...
			// OJO  Only for debugging output_files registry
	...
	if ($debug){
		print "<br/><br/>FINAL FILES PENDING <br/>";
...
function  build_outputs_list($tool,$stageout_file){
...
//                var_dump($out_data['file']);
		...
		if (!isset($stageout_meta[$out_name])){
			//print "Tool stageout file '".basename($stageout_file)."' has no metadata for output_file '$out_name'.";
...
/*
			//validate new file data
			$mugFile = validateMugFile($file_merged,true);

                	print "<br/><br> VALIDATED FILE <br/>";
	                var_dump($mugFile);

			if (!$mugFile){
				$_SESSION['errorData']['Error'][]="Tool outfile '$out_name' cannot be registered. Missing required metadata";
				print "ERROR<br>";
				var_dump($_SESSION['errorData']);
				continue;
			}
