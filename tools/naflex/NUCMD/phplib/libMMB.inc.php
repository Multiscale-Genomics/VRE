...
function headerMMB($title, $user=array(),$menu=True) {
    $userData='Anonymous';
    $headerLogged = "headerLogged.inc.htm";
    if (preg_match("/MuG/",$_SERVER[REQUEST_URI])){
	$headerLogged = "headerLoggedMuG.inc.htm";
    }
...
#var_dump($_SESSION);
	...
	#echo "UserData: $userData";
        return parseTemplate(
                    Array('title' => $title, 'userData' => $userData, 'userId' => $userId, 'menu' => $menu, 'homeURL' => $GLOBALS['homeURL']), getTemplate($headerLogged)
            );
    }
    else
    	return parseTemplate(
