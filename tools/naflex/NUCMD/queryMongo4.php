...
switch ($block) {
		...
		<?php
			...
			$resultsMg = $analCount->aggregate( $fcond ); 
			$results = 0;
			if($resultsMg['result'])
			        $results = $resultsMg['result'][0]['total'];
...
#print "</pre>";
	...
	case "2":
		...
		if ( preg_match('/HBs/',$mongoId) or preg_match('/NMR/',$mongoId)){
			...
			$anal = preg_replace('/_avg/','',$anal);

                        $fcond = buildMongoQueryOld($regexp,$anal,$trajs);
...
#print "</pre>";
			...
			$type = preg_replace('/-/','',$type);
			$regexp = buildRegExp("_id.idGroup",$type,$nucType);
			$reg = new MongoRegex("/^$anal\.\w+$/");

			$fcond = buildMongoQuery($regexp,$reg,$trajs);

			$results = $analCount->aggregate( $fcond ); 

			$analHash = array();
			if(! empty($results)){
      		               	foreach ($results['result'] as $mganal) {

					$mg_id = $mganal['_id'];
					$mg_id = end(explode(".",$mg_id));

					$mg_total = $mganal['total'];
					$analHash[$mg_id] = $mg_total;
				}
			}
...
#print "</pre>";
	...
	case "3":
		...
		$type = preg_replace('/-/','',$type);
		$regexp = buildRegExp("_id.idGroup",$type,$nucType);
		$reg = new MongoRegex("/^$anal\.$subanal\.\w+$/");
...
logger(json_encode(array("$anal.$subanal" => 1)));
		...
		$fcond = buildMongoQueryOld($regexp,"$anal.$property",$trajs);

#print "<pre>";
#print "Block: $block, Id ($i): ".$mongoId."($property)<br/>";
#print json_encode($fcond);
#print "</pre>";
	...
	case "4":
		...
		$fcond = buildMongoQueryOld($regexp,"$anal.$subanal.$property",$trajs);
...
function buildMongoQueryOld ($regexp,$toSearch,$trajs)
{
	$cond = Array();
	$cond[] = $regexp;
...
{
	# db.analCount.aggregate([ { $match: {'_id.idGroup':/^AA/,'_id.anal':'CURVES','_id.idSim':{$in:['NAFlex_DDD_bsc1','NAFlex_1hlv']}}}, { $group : {'_id':null,  total: {$sum : "$count"}}}    ])
	$cond = Array();
	$cond[] = $regexp;
	$cond[] = array('_id.anal' => $toSearch);
	if(!empty($trajs)){
                $in = array('_id.idSim' => array('$in' => $trajs));
       		$cond[] = $in;
	}
	...
	$aggCond = Array();
	$aggCond[] = array('$match' => $fcond);
	$aggCond[] = array('$group' => array ('_id' => '$_id.anal', 'total' => array('$sum' => '$count')));
	return $aggCond;
}
